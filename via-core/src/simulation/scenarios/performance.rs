    ...
    fn tick(&mut self, current_time_ns: u64, delta_ns: u64) -> Vec<LogRecord> {
        if self.has_crashed {
            ...
            return vec![create_log(
                ...
                current_time_ns,
                vec![KeyValue { key: "event".to_string(), value: AnyValue::string("service_start") }]
            )];
        }
        ...
        if rng.random_bool(0.2) { // not every tick, but frequent
             ...
             let level = if self.current_memory_mb > self.max_memory_mb * 0.9 { "FATAL" } 
                         else if self.current_memory_mb > self.max_memory_mb * 0.75 { "WARN" } 
                         else { "INFO" };
            
             let body = format!("Memory usage: {:.2} MB / {:.2} MB", self.current_memory_mb, self.max_memory_mb);
             ...
             logs.push(create_log(
                ...
                current_time_ns,
                vec![
                    KeyValue { key: "process.memory.usage".to_string(), value: AnyValue::double(self.current_memory_mb) },
                    KeyValue { key: "process.memory.limit".to_string(), value: AnyValue::double(self.max_memory_mb) },
                ]
             ));
        }
        ...
        if self.current_memory_mb >= self.max_memory_mb {
            ...
            logs.push(create_log(
                ...
                &Uuid::new_v4().simple().to_string()[..16],
                current_time_ns,
                vec![KeyValue { key: "error.type".to_string(), value: AnyValue::string("OOM") }]
            ));
        }
...
impl CpuSpike {
    pub fn new(service_name: &str, intensity: f64) -> Self {
        Self { service_name: service_name.to_string(), intensity }
    }
}
...
impl Scenario for CpuSpike {
    ...
    fn tick(&mut self, current_time_ns: u64, _delta_ns: u64) -> Vec<LogRecord> {
        ...
        if rng.random_bool(self.intensity) {
             ...
             logs.push(create_log(
                "WARN",
                format!("Thread pool exhaustion: Active threads > 95% (Processing time: {}ms)", duration),
                &self.service_name,
                &trace_id,
                &span_id,
                current_time_ns,
                vec![
                    KeyValue { key: "process.cpu.utilization".to_string(), value: AnyValue::double(99.9) },
                    KeyValue { key: "thread.active_count".to_string(), value: AnyValue::int(200) },
                ]
             ));
        }
        logs
    ...
    fn tick(&mut self, current_time_ns: u64, _delta_ns: u64) -> Vec<LogRecord> {
        ...
        if rng.random_bool(0.05) { 
            ...
            vec![create_log(
                ...
                &span_id,
                current_time_ns,
                vec![KeyValue { key: "error.stack_depth".to_string(), value: AnyValue::int(1024) }]
            )]
        } else {
            vec![]
        }
