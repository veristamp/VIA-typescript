...
use via_sim::{AnyValue, LogRecord};
use via_sim::{
    SimulationEngine,
    scenarios::{performance::CpuSpike, traffic::NormalTraffic},
};
...
fn main() {
    ...
    for t in 0..(total_duration_s * (1000 / tick_ms)) {
        ...
        if t % (1000 / tick_ms) == 0 {
            let avg_lat = if batch_count > 0 {
                batch_latency_sum / batch_count as f64
            } else {
                0.0
            };
            ...
            print!(
                "\r{:<10.1} | {:<10} | {:<10} | {:<15.2} | {:<10}",
                current_sim_time_s,
                total_events,
                batch_count * (1000 / tick_ms) as usize,
                avg_lat,
                status
            );
