...
impl Detector for CardinalityDetectorV2 {
    ...
    fn get_stats(&self) -> String {
        let (mean, _std, thresh, _count) = self.adaptive_threshold.get_stats();
        format!(
...
impl SpectralDetector {
    pub fn new() -> Self {
        Self {
            ...
            _value_buffer: Vec::with_capacity(24),
...
impl AnomalyProfile {
    ...
    /// Process with string ID (converts to hash internally)
...
pub fn create_profile(
    _profile_type: AnomalyProfileType,
    hw_alpha: f64,
    hw_beta: f64,
    hw_gamma: f64,
    period: usize,
